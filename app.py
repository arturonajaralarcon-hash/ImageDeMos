...
try:
    ...
    st.warning("⚠️ Configura 'PASSWORD_ACCESO' en .streamlit/secrets.toml")
...
if check_password():
    ...
    if st.session_state.historial:
        ...
        # Grid de 4 columnas para tener espacio para los botones
        cols = st.columns(4)
        ...
        for i, img in enumerate(st.session_state.historial):
            with cols[i % 4]:
                ...
                # --- BOTONERA DE ACCIONES POR IMAGEN ---
                ...
                if c2.button("🔍 4K", key=f"up_{i}", help="Procesar a 4K"):
                    with st.spinner("Reescalando..."):
                        ...
                        # Hack para descargar el archivo procesado al instante
                        st.session_state[f"ready_4k_{i}"] = buf_4k.getvalue()
                        st.rerun()
                
                # Mostrar botón de descarga 4K si ya se procesó
                if f"ready_4k_{i}" in st.session_state:
                    c2.download_button("⬇️ 4K", st.session_state[f"ready_4k_{i}"], f"demos_4k_{i}.png", "image/png", key=f"dl4k_{i}")

                # 3. Mover a Referencias
                if c3.button("🔄 Ref", key=f"ref_{i}", help="Usar como referencia"):
                    ...
                    st.toast("Imagen añadida a Referencias", icon="✅")
                    time.sleep(0.5)
